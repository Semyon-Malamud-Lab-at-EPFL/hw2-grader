...
sys.path.insert(0, STUDENT); sys.path.insert(0, GRADER)
...
def main():
    ...
    except Exception as e: traceback.print_exc(); t2=[(0,12,str(e))]
    tasks.append(("Task 2: EDA", t2))

    print("[3] Split ...")
    try: t3, r_split = test_task3(s_dl, ref, r_df, r_fc, TARGET_COL, N_TRAIN)
    except Exception as e:
        traceback.print_exc()
    ...
    except Exception as e: traceback.print_exc(); t7=[(0,20,str(e))]
    tasks.append(("Task 7: Portfolio", t7))

    summary = []
    for name, results in tasks:
        pts=sum(r[0] for r in results); mx=sum(r[1] for r in results)
        summary.append({"name":name,"points":round(pts,2),"max_points":round(mx,2)})
    ...
    report = {"total_points":round(total,2),"max_points":round(total_max,2),
